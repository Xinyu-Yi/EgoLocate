...
public:
    ...
    // TransPose camera pose estimation
    bool use_transpose = false;
    Sophus::SE3f transpose_pose;   // Tjj'  transpose estimation in transpose coordinate frame
    Sophus::SE3f local_transpose_pose;  // Tcw.  aligned transpose estimation in SLAM coordinate frame. set in Tracking::InitializePoseUsingTransPose(), read when tracking fail
    Sophus::SE3f local_transpose_pose_not_align;  // Tcw. transpose estimation in SLAM coordinate frame. set and read in Tracking::InitializePoseUsingTransPose()
    vector<KeyFrame*> vpCandidateKFs;   // Candidate KFs used in constructing Local Map. Only set in Tracking::Relocalization(). Read and destroy in TrackLocalMap. No need to copy.
