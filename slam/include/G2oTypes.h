...
class TransPoseCamPose
{
public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW
    TransPoseCamPose(){}
    TransPoseCamPose(KeyFrame* pKF);
    TransPoseCamPose(Frame* pF);
...
public:
    ...
    int its;  // how many times are update() called, used to normalize rotation matrix
};
...
public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW
    VertexSO3(){}
    VertexSO3(const Eigen::Matrix3d &init){
        setEstimate(Sophus::SO3d(NormalizeRotation(init)));
    }
    ...
    virtual void oplusImpl(const double* update_){
        _estimate = Sophus::SO3d::exp(Eigen::Vector3d(update_[0], update_[1], update_[2])) * _estimate;
        updateCache();
    }
};
...
class VertexTranBias: public g2o::BaseVertex<3, Eigen::Vector3d>
{
public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW
    VertexTranBias(const Eigen::Vector3d &init){ setEstimate(init); }
    ...
    virtual void oplusImpl(const double* update_){
        _estimate += Eigen::Vector3d(update_[0], update_[1], update_[2]);
        updateCache();
    }
};


class VertexPoseBias: public g2o::BaseVertex<3, Eigen::Matrix3d>
{
public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW
    VertexPoseBias(){ setEstimate(Eigen::Matrix3d::Identity()); }
    VertexPoseBias(const Eigen::Matrix3d &init){ setEstimate(init); }
    ...
    virtual void oplusImpl(const double* update_){
        _estimate = _estimate * Sophus::SO3d::exp(Eigen::Vector3d(update_[0], update_[1], update_[2])).matrix();
        updateCache();
    }
};
...
class VertexPoseWithTransPose : public g2o::BaseVertex<6,TransPoseCamPose>
{
public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW
    VertexPoseWithTransPose(){}
    VertexPoseWithTransPose(KeyFrame* pKF){
        setEstimate(TransPoseCamPose(pKF));
    }
    VertexPoseWithTransPose(Frame* pF){
        setEstimate(TransPoseCamPose(pF));
    }
...
class EdgeRARTBT : public g2o::BaseUnaryEdge<3, Eigen::Matrix3d, VertexSO3>
{
public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW
    EdgeRARTBT(const Eigen::Matrix3d &A_) { A = NormalizeRotation(A_); }
    virtual bool read(std::istream& is){return false;}
    virtual bool write(std::ostream& os) const{return false;}

    void setMeasurement(const Measurement& m) { _measurement = NormalizeRotation(m); }

    void computeError() {
        VertexSO3* v1 = static_cast<VertexSO3*>(_vertices[0]);
        Eigen::Matrix3d R = v1->estimate().matrix();
        _error = Sophus::SO3d(R * A * R.transpose() * _measurement.transpose()).log();
    }

    // virtual void linearizeOplus(); //todo: impl
...
class EdgeRAT : public g2o::BaseUnaryEdge<3, Eigen::Matrix3d, VertexSO3>
{
public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW
    EdgeRAT() {}
    ...
    void computeError() {
        VertexSO3* v1 = static_cast<VertexSO3*>(_vertices[0]);
        Eigen::Matrix3d R = v1->estimate().matrix();
        _error = Sophus::SO3d(R * _measurement.transpose()).log();
    }

    // virtual void linearizeOplus(); //todo: impl
};

class VertexSim3OnlyRotation : public g2o::BaseVertex<3, Sophus::Sim3d>
{
public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW
    VertexSim3OnlyRotation() {}
    virtual bool read(std::istream& is){return false;}
    virtual bool write(std::ostream& os) const{return false;}
    ...
    virtual void oplusImpl(const double* update_)
    {
        ...
        Eigen::Matrix3d R = Sophus::SO3d::exp(update).matrix() * _estimate.rotationMatrix();
        _estimate.setRotationMatrix(R);
        updateCache();
    }
};


class EdgeSim3PointPointOnlyRotation : public g2o::BaseUnaryEdge<3, Eigen::Vector3d, VertexSim3OnlyRotation>
{
public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW
    EdgeSim3PointPointOnlyRotation(const Eigen::Vector3f &Xc_) : Xc(Xc_.cast<double>()){}
    virtual bool read(std::istream& is){return false;}
    virtual bool write(std::ostream& os) const{return false;}

    void computeError() {
        VertexSim3OnlyRotation* v1 = static_cast<VertexSim3OnlyRotation*>(_vertices[0]);
        _error = v1->estimate() * Xc - _measurement;
    }
...
class EdgeSim3OnlyRotation : public g2o::BaseUnaryEdge<3, Eigen::Matrix3d, VertexSim3OnlyRotation>
{
public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW
    EdgeSim3OnlyRotation() {}
    virtual bool read(std::istream& is){return false;}
    virtual bool write(std::ostream& os) const{return false;}

    void computeError() {
        VertexSim3OnlyRotation* v1 = static_cast<VertexSim3OnlyRotation*>(_vertices[0]);
        _error = Sophus::SO3d(v1->estimate().rotationMatrix() * _measurement.transpose()).log();
    }

    // virtual void linearizeOplus(); //todo: impl
};

class EdgeSim3PointPoint : public g2o::BaseUnaryEdge<3, Eigen::Vector3d, g2o::VertexSim3Expmap>
{
public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW
    EdgeSim3PointPoint(const Eigen::Vector3f &Xc_) : Xc(Xc_.cast<double>()){}
    virtual bool read(std::istream& is){return false;}
    virtual bool write(std::ostream& os) const{return false;}

    void computeError() {
        g2o::VertexSim3Expmap* v1 = static_cast<g2o::VertexSim3Expmap*>(_vertices[0]);
        _error = v1->estimate().map(Xc) - _measurement;
    }
...
class EdgeMonoWithTransPose : public g2o::BaseBinaryEdge<2,Eigen::Vector2d,g2o::VertexSBAPointXYZ,VertexPoseWithTransPose>
{
public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW
    virtual bool read(std::istream& is){return false;}
    ...
    void computeError() {
        const g2o::VertexSBAPointXYZ* VPoint = static_cast<const g2o::VertexSBAPointXYZ*>(_vertices[0]);
        const VertexPoseWithTransPose* VPose = static_cast<const VertexPoseWithTransPose*>(_vertices[1]);
        const Eigen::Vector2d obs(_measurement);
...
class EdgeTransPoseHeight : public g2o::BaseUnaryEdge<1, double, VertexPoseWithTransPose>
{
public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW
    EdgeTransPoseHeight() {}
    ...
    void computeError() {
        const VertexPoseWithTransPose* VPose = static_cast<const VertexPoseWithTransPose*>(_vertices[0]);
        Eigen::Vector3d tran = SMC0 * VPose->estimate().twc;
        _error(0, 0) = tran.y() - _measurement;
    }

    static Sophus::Sim3d SMC0;

    // virtual void linearizeOplus();  //todo:maybe important, and change SMC0 as static
};


class EdgeMonoOnlyPoseWithTransPose : public g2o::BaseUnaryEdge<2,Eigen::Vector2d,VertexPoseWithTransPose>
{
public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW
    EdgeMonoOnlyPoseWithTransPose(const Eigen::Vector3f &Xw_) : Xw(Xw_.cast<double>()){}
    virtual bool read(std::istream& is){return false;}
    virtual bool write(std::ostream& os) const{return false;}

    void computeError() {
        const VertexPoseWithTransPose* VPose = static_cast<const VertexPoseWithTransPose*>(_vertices[0]);
        _error = _measurement - VPose->estimate().Project(Xw);
    }
...
class EdgeTransPosePose : public g2o::BaseUnaryEdge<1, Eigen::Matrix3d, VertexPoseWithTransPose>
{
public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW
    virtual bool read(std::istream& is){return false;}
    ...
    void computeError() {  // || log(R^T Rwc)^\vee ||_2
        const VertexPoseWithTransPose* VPose = static_cast<const VertexPoseWithTransPose*>(_vertices[0]);
        Sophus::SO3d dR(_measurement.transpose() * VPose->estimate().Rwc);
        double theta = dR.logAndTheta().theta;
        _error(0, 0) = theta;
    }
...
class EdgeTransPosePoseFull : public g2o::BaseUnaryEdge<3, Eigen::Matrix3d, VertexPoseWithTransPose>
{
public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW
    virtual bool read(std::istream& is){return false;}
    ...
    void computeError() {  // og(R^T Rwc)^\vee
        const VertexPoseWithTransPose* VPose = static_cast<const VertexPoseWithTransPose*>(_vertices[0]);
        Sophus::SO3d dR(_measurement.transpose() * VPose->estimate().Rwc);
        _error = dR.log();
    }
...
class EdgeTransPoseTrans : public g2o::BaseUnaryEdge<3, Eigen::Vector3d, VertexPoseWithTransPose>
{
public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW
    virtual bool read(std::istream& is){return false;}
    ...
    void computeError() {  // t - twc
        const VertexPoseWithTransPose* VPose = static_cast<const VertexPoseWithTransPose*>(_vertices[0]);
        _error= _measurement - VPose->estimate().twc;
    }

    virtual void linearizeOplus() {
        const VertexPoseWithTransPose* VPose = static_cast<const VertexPoseWithTransPose*>(_vertices[0]);
        Eigen::Matrix<double, 3, 6> J = Eigen::Matrix<double, 3, 6>::Zero();
        J.block<3, 3>(0, 3) = -VPose->estimate().Rwc;
        _jacobianOplusXi = J;
    }
};


class EdgePoseBiasNear : public g2o::BaseUnaryEdge<3, Eigen::Matrix3d, VertexPoseBias>
{
public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW
    virtual bool read(std::istream& is){return false;}
    ...
    void computeError() {
        const VertexPoseBias* v0 = static_cast<const VertexPoseBias*>(_vertices[0]);
        Sophus::SO3d dR(_measurement.transpose() * v0->estimate());
        _error = dR.log();
    }

    // virtual void linearizeOplus() {
    //     const VertexPoseWithTransPose* v0 = static_cast<const VertexPoseWithTransPose*>(_vertices[0]);
    //     const VertexPoseWithTransPose* v1 = static_cast<const VertexPoseWithTransPose*>(_vertices[1]);
    //     _jacobianOplusXi = Eigen::Matrix<double, 3, 6>::Zero();
    //     _jacobianOplusXi.block<3, 3>(0, 3) = v0->estimate().Rwc;
    //     _jacobianOplusXj = Eigen::Matrix<double, 3, 6>::Zero();
    //     _jacobianOplusXj.block<3, 3>(0, 3) = -v1->estimate().Rwc;
    // }
};

class EdgeTranBiasNear : public g2o::BaseUnaryEdge<3, Eigen::Vector3d, VertexTranBias>
{
public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW
    virtual bool read(std::istream& is){return false;}
    ...
    void computeError() {
        const VertexTranBias* v0 = static_cast<const VertexTranBias*>(_vertices[0]);
        _error= _measurement - v0->estimate();
    }
...
class EdgeTransPoseTransBetweenTwoVertexWithBias : public g2o::BaseMultiEdge<3, Eigen::Vector3d>
{
public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW
    EdgeTransPoseTransBetweenTwoVertexWithBias() {resize(3);}
    virtual bool read(std::istream& is){return false;}
    virtual bool write(std::ostream& os) const{return false;}

    void computeError() {  // measurement - (v[1] - v[0]) - bias
        const VertexPoseWithTransPose* v0 = static_cast<const VertexPoseWithTransPose*>(_vertices[0]);
        const VertexPoseWithTransPose* v1 = static_cast<const VertexPoseWithTransPose*>(_vertices[1]);
        const VertexTranBias *v2 = static_cast<const VertexTranBias*>(_vertices[2]);
        _error= _measurement - v1->estimate().twc + v0->estimate().twc - v0->estimate().Rwc * v2->estimate();  // fixme: should use root orientation not Rwc
    }
...
class EdgeTransPosePoseFullWithBias : public g2o::BaseBinaryEdge<3, Eigen::Matrix3d, VertexPoseWithTransPose, VertexPoseBias>
{
public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW
    virtual bool read(std::istream& is){return false;}
    ...
    void computeError() {  // og(R^T Rwc)^\vee
        const VertexPoseWithTransPose* VPose = static_cast<const VertexPoseWithTransPose*>(_vertices[0]);
        const VertexPoseBias* VPB = static_cast<const VertexPoseBias*>(_vertices[1]);
        Sophus::SO3d dR(_measurement.transpose() * VPose->estimate().Rwc * VPB->estimate());
        _error = dR.log();
    }
...
class EdgeTransPoseTransBetweenTwoVertex : public g2o::BaseBinaryEdge<3, Eigen::Vector3d, VertexPoseWithTransPose, VertexPoseWithTransPose>
{
public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW
    virtual bool read(std::istream& is){return false;}
    ...
    void computeError() {  // measurement - (v[1] - v[0])
        const VertexPoseWithTransPose* v0 = static_cast<const VertexPoseWithTransPose*>(_vertices[0]);
        const VertexPoseWithTransPose* v1 = static_cast<const VertexPoseWithTransPose*>(_vertices[1]);
        _error= _measurement - v1->estimate().twc + v0->estimate().twc;
    }
