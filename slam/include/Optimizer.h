...
{
private:
    void static ShowErrorOnDifferentEdge(g2o::SparseOptimizer &optimizer);  // for debug
public:
    float static SjcScale;
    static Eigen::Vector3d biasTran;
    static Eigen::Matrix3d biasPose;
    static double kAbsoluteTranEdgeGlobalBA, kPoseEdgeGlobalBA, kTranEdgeGlobalBA, kPoseEdgeLocalBA, kTranEdgeLocalBA, kPoseEdgeTrack, kTranEdgeTrack;
    static bool debugOptimization, trustPIPduringMapping;
    static double ky;
    ...
    void static FullBAWithTransPose(Map *pMap, int its, const bool bFixLocal, const long unsigned int nLoopId, bool *pbStopFlag);
    ...
    void static SMC0Optimization(Tracking *pTracker, const vector<bool> *pvLocalMask = NULL);
    Eigen::Matrix3f static RIBRHCOptimization(const Eigen::Matrix3f &A, const Eigen::Matrix3f &B, const Eigen::Matrix3f &R_init);
    Eigen::Matrix3f static RIBRHCOptimization(const vector<KeyFrame *> &vpKFs, const Eigen::Matrix3f &R_init, const Eigen::Matrix3f &RMH0);
    int static PoseOptimization(Frame* pFrame);
    int static PoseOptimizationWithTransPose(Frame* pFrame);
    int static PoseInertialOptimizationLastKeyFrame(Frame* pFrame, bool bRecInit = false);
    int static PoseInertialOptimizationLastFrame(Frame *pFrame, bool bRecInit = false);

    void static OptimizeEssentialGraphWithTransPose(Map* pMap, KeyFrame* pLoopKF, KeyFrame* pCurKF,
    ...
    void static LocalBAWithTransPose(KeyFrame* pKF, bool *pbStopFlag, Map *pMap, bool bLarge = false, bool bFlatFloor = false);
    void static LocalBAWithTransPoseNew(KeyFrame* pKF, bool *pbStopFlag, Map *pMap, bool bLarge = false, bool bFlatFloor = false);
    void static LocalBAWithTransPoseNewWithBias(KeyFrame* pKF, bool *pbStopFlag, Map *pMap, bool bLarge = false, bool bFlatFloor = false);
