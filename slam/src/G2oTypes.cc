...
TransPoseCamPose::TransPoseCamPose(KeyFrame *pKF):its(0)
{
    Rcw = pKF->GetPose().rotationMatrix().cast<double>();
    tcw = pKF->GetPose().translation().cast<double>();
    Rcw = NormalizeRotation(Rcw);
    Rwc = Rcw.transpose();
    twc = -Rwc * tcw;
    pCamera = pKF->mpCamera;
}

TransPoseCamPose::TransPoseCamPose(Frame *pF):its(0)
{
...
Eigen::Vector2d TransPoseCamPose::Project(const Eigen::Vector3d &Xw) const
{
    Eigen::Vector3d Xc = Rcw * Xw + tcw;
    return pCamera->project(Xc);
}

bool TransPoseCamPose::isDepthPositive(const Eigen::Vector3d &Xw) const
{
    return (Rcw.row(2) * Xw + tcw(2)) > 0.0;
}

void TransPoseCamPose::Update(const double *pu)
{
...
void EdgeMonoWithTransPose::linearizeOplus()
{
...
void EdgeTransPosePose::linearizeOplus()
{
    const VertexPoseWithTransPose* VPose = static_cast<const VertexPoseWithTransPose*>(_vertices[0]);
    Eigen::Matrix3d dR = _measurement.transpose() * VPose->estimate().Rwc;
    double a = dR(0, 0);
    double b = dR(0, 1);
    double c = dR(0, 2);
    double d = dR(1, 0);
    ...
    double h = dR(2, 1);
    double i = dR(2, 2);
    double k = 1 / sqrt(4 - (a + e + i - 1) * (a + e + i - 1));
    Eigen::Matrix<double, 1, 6> J;
    J << h - f, c - g, d - b, 0, 0, 0;
    _jacobianOplusXi = k * J;
}

Sophus::Sim3d EdgeTransPoseHeight::SMC0;

void EdgeTransPosePoseFull::linearizeOplus()
{
    computeError();
    double theta = _error.norm();
    double theta2 = theta / 2;
    double cot = 1 / tan(theta2);
    Eigen::Vector3d a = -_error / theta;
    Eigen::Matrix<double, 3, 6> J;
    J.block<3, 3>(0, 0) = theta2 * cot * Eigen::Matrix3d::Identity() + (1 - theta2 * cot) * a * a.transpose() - theta2 * Sophus::SO3d::hat(a);
    J.block<3, 3>(0, 3) = Eigen::Matrix3d::Zero();
    _jacobianOplusXi = J;
}


void EdgeMonoOnlyPoseWithTransPose::linearizeOplus()
{
    const VertexPoseWithTransPose* VPose = static_cast<const VertexPoseWithTransPose*>(_vertices[0]);
