...
KeyFrame::KeyFrame():
        ...
        NLeft(0),NRight(0), mnNumberOfOpt(0), mbHasVelocity(false), use_transpose(false)
{
...
KeyFrame::KeyFrame(Frame &F, Map *pMap, KeyFrameDatabase *pKFDB):
    ...
    local_transpose_pose(F.local_transpose_pose), local_transpose_pose_not_align(F.local_transpose_pose_not_align),
    transpose_pose(F.transpose_pose), use_transpose(F.use_transpose)
{
    mnId=nNextId++;
...
void KeyFrame::ChangeParent(KeyFrame *pKF)
{
    ...
    mpParent = pKF;
    if (pKF)
        pKF->AddChild(this);   // original system has bug when pKF == NULL
}
