    ...
    while(1)
    {
        ...
        if(CheckNewKeyFrames())
        {
            ...
            if(bFindedRegion)
            {
                if(mbMergeDetected)
                {
                    Verbose::PrintMess("\n[Loop Closing] Map merge detected", Verbose::VERBOSITY_LOOP_CLOSING);
                ...
                if(mbLoopDetected)
                {
                    Verbose::PrintMess("\n[Loop Closing] Loop detected", Verbose::VERBOSITY_LOOP_CLOSING);
                    bool bGoodLoop = true;
                    vdPR_CurrentTime.push_back(mpCurrentKF->mTimeStamp);
...
void LoopClosing::CorrectLoop()
{
    ...
    if (mpTracker->mCurrentFrame.use_transpose) {
        Verbose::PrintMess("[Local Mapping] Optimize essential graph with TransPose", Verbose::VERBOSITY_LOOP_CLOSING);
        Optimizer::OptimizeEssentialGraphWithTransPose(pLoopMap, mpLoopMatchedKF, mpCurrentKF, NonCorrectedSim3, CorrectedSim3, LoopConnections, true);
    }
    else if(pLoopMap->IsInertial() && pLoopMap->isImuInitialized())
    {
        Verbose::PrintMess("[Local Mapping] Optimize essential graph 4DOF", Verbose::VERBOSITY_LOOP_CLOSING);
        Optimizer::OptimizeEssentialGraph4DoF(pLoopMap, mpLoopMatchedKF, mpCurrentKF, NonCorrectedSim3, CorrectedSim3, LoopConnections);
    }
    else
    {
        Verbose::PrintMess("[Local Mapping] Optimize essential graph monocular", Verbose::VERBOSITY_LOOP_CLOSING);
    ...
    if((!pLoopMap->isImuInitialized() && !mpCurrentKF->use_transpose) || (pLoopMap->KeyFramesInMap()<200 && mpAtlas->CountMaps()==1))
    {
        mbRunningGBA = true;
        mbFinishedGBA = false;
        mbStopGBA = false;
...
{  
    ...
    if (mpTracker->mCurrentFrame.use_transpose) {
        Verbose::PrintMess("[Local Mapping] Starting global bundle adjustment with TransPose", Verbose::VERBOSITY_LOOP_CLOSING);
        Optimizer::FullBAWithTransPose(pActiveMap, 7, false, nLoopKF, &mbStopGBA);  // todo: fix local?
    }
    else if(!bImuInit) {
        Verbose::PrintMess("[Local Mapping] Starting global bundle adjustment monocular", Verbose::VERBOSITY_LOOP_CLOSING);
        Optimizer::GlobalBundleAdjustemnt(pActiveMap,10,&mbStopGBA,nLoopKF,false);
    }
    else {
        Verbose::PrintMess("[Local Mapping] Starting global bundle adjustment inertial", Verbose::VERBOSITY_LOOP_CLOSING);
