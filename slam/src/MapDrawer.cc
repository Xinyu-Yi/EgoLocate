...
void MapDrawer::DrawReferenceCamera(pangolin::OpenGlMatrix &Twc) {
    const float &w = mCameraSize;
    const float h = w*0.75;
    const float z = w*0.6;
...
void MapDrawer::SetReferenceCameraPose(const Sophus::SE3f &Tcw) {
    unique_lock<mutex> lock(mMutexCamera);
    mRefCameraPose = Tcw.inverse();
}

void MapDrawer::GetCurrentOpenGLCameraMatrix(pangolin::OpenGlMatrix &M, pangolin::OpenGlMatrix &MOw, pangolin::OpenGlMatrix &Mref)
{
    Eigen::Matrix4f Twc, Twcref;
    {
        unique_lock<mutex> lock(mMutexCamera);
        Twc = mCameraPose.matrix();
        Twcref = mRefCameraPose.matrix();
    }
    ...
    for (int i = 0; i<4; i++) {
        Mref.m[4*i] = Twcref(0,i);
        Mref.m[4*i+1] = Twcref(1,i);
        Mref.m[4*i+2] = Twcref(2,i);
        Mref.m[4*i+3] = Twcref(3,i);
    }
